...
(*                                                                     *)
(***********************************************************************)
(* Contributors: * Fabrice LE FESSANT (INRIA/OCamlPro) *)
...
(* TODO:
   ...
   If a or-pattern contains both "Reg64 ... | Reg32 ... ", it means that we
   didn't discriminate between 32 bit and 64 bit modes for that instruction. It
   also means that using this instruction on a 32-bit register in 64 bit mode
   will not generate the 32-bit version of the instruction, but the 64-bit
   version... *)
...
(* Try to compute some statically computable arithmetic expressions in labels,
   or to simplify them to a form that is encodable by relocations. *)
let eval_const b current_pos cst =
  let rec eval = function
    ...
    | C.Add (c1, c2) -> (
      ...
      (* TODO: we could add another case, easy to solve: adding a Rrel to a Rabs
         where the symbol is local, in which case it can be computed. *)
      | Rrel (s, n1), Rabs ("", n2) -> Rabs (s, Int64.add n1 n2)
      | _ -> assert false)
  in
  try
    let r = eval cst in
    (* if debug then Printf.eprintf "eval_const (%s) = %s at @%d\n%!"
       (X86_gas.string_of_constant cst) (string_of_result r) current_pos; *)
    r
  with e ->
...
let is_imm32L n =
  Int64.compare n 0x8000_0000L < 0 && Int64.compare n (-0x8000_0000L) >= 0
...
let rd_of_regf regf = match regf with XMM n | YMM n | ZMM n -> n
...
(* We should precompute a position for each label depending on the number of
   instructions: heuristics = offset_in_instrs x 7 *)
...
let emit_prefix_modrm b opcodes rm reg ~prefix =
  ...
  | (Reg8L _ | Reg8H _) as reg8 ->
    ...
    prefix b ~rex:(rex_of_reg8 reg8) ~rexr:(rexr_reg reg) ~rexb:(rexb_rm rm)
      ~rexx:0;
    buf_opcodes b opcodes;
    buf_int8 b (mod_rm_reg 0b11 rm reg)
  | Reg16 reg16 ->
    let rm = rd_of_reg64 reg16 in
    prefix b ~rex:(rex_of_reg16 reg16) ~rexr:(rexr_reg reg) ~rexb:(rexb_rm rm)
      ~rexx:0;
    buf_opcodes b opcodes;
    buf_int8 b (mod_rm_reg 0b11 rm reg)
  | Regf regf ->
  ...
  | Mem { arch; typ = _; idx; scale; base; sym; displ } -> (
    let offset =
      ...
      | None ->
        if is_imm8L displ
        then OImm8 displ
        else if is_imm32L displ
        then OImm32 (None, displ)
        else assert false
    ...
    then (
      ...
      | None -> (
        ...
        | (RSP | R12), 1, OImm8 0L ->
          prefix b ~rex:0 ~rexr:(rexr_reg reg) ~rexb:(rexb_base idx) ~rexx:0;
          buf_opcodes b opcodes;
        ...
        | _, 1, OImm8 0L ->
          ...
          prefix b ~rex:0 ~rexr:(rexr_reg reg) ~rexb:(rexb_rm idx) ~rexx:0;
          buf_opcodes b opcodes;
          buf_int8 b (mod_rm_reg 0b00 idx reg)
        | _, 1, OImm8 offset8 ->
          prefix b ~rex:0 ~rexr:(rexr_reg reg) ~rexb:(rexb_rm idx) ~rexx:0;
        ...
        | _, 1, OImm32 (sym, offset) ->
          ...
          buf_sym b sym offset
        | _, _, _ -> (
          prefix b ~rex:0 ~rexr:(rexr_reg reg) ~rexb:0 ~rexx:(rexx_index idx);
          buf_opcodes b opcodes;
      ...
      | Some base_reg -> (
        ...
        prefix b ~rex:0 ~rexr:(rexr_reg reg) ~rexb:(rexb_base base)
          ~rexx:(rexx_index idx);
        buf_opcodes b opcodes;
...
let emit_vex3 buf ~rexr ~rexx ~rexb ~vex_m ~vex_w ~vex_v ~vex_l ~vex_p =
  buf_int8 buf 0xC4;
  (* We only emit 3-byte VEX instructions. *)
  buf_int8 buf
    (((rexr lxor 1) lsl 7)
    lor ((rexx lxor 1) lsl 6)
    lor ((rexb lxor 1) lsl 5)
    lor vex_m);
  buf_int8 buf
    ((vex_w lsl 7) lor ((vex_v lxor 15) lsl 3) lor (vex_l lsl 2) lor vex_p)

let vex_prefix_adaptor f b ~rex:_ ~rexr ~rexb ~rexx =
...
let emit_simd b (instr : Amd64_simd_instrs.instr) args =
  ...
  let imm, args =
    ...
    | Imm_spec | Imm_reg -> Some args.(0), Array.sub args 1 (n - 1)
    | Imm_none -> None, args
  in
  let enc i =
      ...
      let rr =
        Array.fold_right
          (fun ({ enc; _ } : Amd64_simd_defs.arg) acc ->
            match enc with Implicit -> acc | _ -> enc :: acc)
          rr []
        |> Array.of_list
      in
      let n = Array.length rr in
      if i < n then rr.(i) else instr.args.(i - n).enc
  in
  let rm_only () =
    match args with
    | [| dst |] -> (
      match enc 0 with RM_rm -> dst | _ -> failwith instr.mnemonic)
  ...
  in
  let mk_rex = function Rex_none -> no_rex | Rex -> rex | Rex_w -> rexw in
  let vex_map = function Vexm_0F -> 1 | Vexm_0F38 -> 2 | Vexm_0F3A -> 3 in
  ...
  | Reg, Vex { vex_m; vex_w; vex_l; vex_p } ->
    ...
    emit_vex_rm_reg b [instr.enc.opcode] rm reg ~vex_m:(vex_map vex_m) ~vex_w
      ~vex_v ~vex_l ~vex_p:(vex_prefix vex_p)
  | Spec rmod, Vex { vex_m; vex_w; vex_l; vex_p } ->
    let rm, vex_v = rm_vexv () in
    emit_vex_rm_reg b [instr.enc.opcode] rm rmod ~vex_m:(vex_map vex_m) ~vex_w
      ~vex_v ~vex_l ~vex_p:(vex_prefix vex_p));
  match imm with
...
let emit_shift reg b dst src =
  ...
  | _ ->
    Format.eprintf "emit_shift: src=%a dst=%a@." print_old_arg src print_old_arg
      dst;
    assert false
...
let emit_reloc_jump near_opcodes far_opcodes b loc symbol =
  if String.Tbl.mem local_labels symbol
  then
    (* local_reloc *)
    ...
    then (
      (* backward *)
      (* The target position is known, and so is the actual offset. We can thus
         decide locally if a short jump can be used. *)
      let target_pos =
      ...
      (* Printf.printf "%s/%i: backward togo_short=%Ld\n%!" symbol loc
         togo_short; *)
      if Int64.compare togo_short (-128L) >= 0
      ...
      (* forward *)
      (* Is the target too far forward (in term of instruction count) or have we
         detected previously that this jump instruction needs to be a long one?

         The str_size constant (see below) is chosen to avoid a second pass most
         oftenm while not being overly pessimistic. *)

      (* if Int64.of_int ((target_loc - loc) * !instr_size) >= 120L then
         Printf.printf "%s/%i: probably too far (%i)\n%!" symbol loc target_loc
         else if IntSet.mem loc !forced_long_jumps then Printf.printf "%s/%i:
         forced long jump\n%!" symbol loc else Printf.printf "%s/%i: short\n%!"
         symbol loc; *)
      let force_far =
        Int64.compare (Int64.of_int ((target_loc - loc) * !instr_size)) 120L
        >= 0
...
let emit_call b dst =
  ...
  | Sym symbol ->
    buf_int8 b 0xE8;
    if String.Tbl.mem local_labels symbol
    then record_local_reloc b (RelocCall symbol)
    else
      (* external symbol, must reloc *)
      record_reloc b (Buffer.length b.buf) (Relocation.Kind.REL32 (symbol, 0L));
    buf_int32L b 0L
...
let emit_LEA b dst src =
  ...
  (* | Reg16 reg, (Mem _ | Mem64_RIP _ as rm) -> let reg = rd_of_reg64 reg in
     emit_mod_rm_reg b 0 [ 0x8D ] rm reg *)
  | _ ->
...
let emit_lock_op ~ops b dst src =
  let rex, rm, reg =
    match dst, src with
    | ((Mem _ | Mem64_RIP _) as rm), Reg64 reg -> rexw, rm, rd_of_reg64 reg
    | ((Mem _ | Mem64_RIP _) as rm), Reg32 reg -> no_rex, rm, rd_of_reg64 reg
    | _ ->
      Misc.fatal_errorf "lock op src=%a dst=%a@." print_old_arg src
        print_old_arg dst
  in
  buf_int8 b 0xF0;
...
let rd_of_prefetch_hint = function Nta -> 0 | T0 -> 1 | T1 -> 2 | T2 -> 3
...
let emit_prefetch b ~is_write ~hint rm =
  ...
  | true, (T1 | T2 | Nta), (Mem _ | Mem64_RIP _) ->
    (* PREFETCHWT1 m8 *)
    (* This sticks to X86_gas' behaviour which emit prefetchwt1 if hint is [T2 |
       Nta] *)
...
let assemble_line b loc ins =
  ...
  with e ->
    ...
    (* Printf.eprintf " masm: %s%!" (string_of_buffer X86_masm.bprint_instr
       !arch64 ins); Printf.eprintf " gas : %s%!" (string_of_buffer
       X86_gas.bprint_instr !arch64 ins); *)
    raise e
  ...
  let rec iter_assemble () =
    ...
    let do_local_reloc pos = function
      | RelocShortJump (label, loc) ->
        ...
        if n >= -128 && n < 128
        then add_patch b pos B8 (Int64.of_int n)
        else (
          (* We thought this could be a short jump, but actually, this is not
             the case. Force another pass and remember to use a long jump for
             this instruction. *)
      ...
      (* TODO: here, we resolve all computations in each section, i.e. we can
         only allow one external symbol per expression. We could tolerate more
         complex expressions if we delay resolution later, i.e. after all
         sections have been generated and all symbol positions are known. *)
      | RelocConstant (cst, data_size) -> (
        (* Printf.eprintf "RelocConstant (%s, %s)\n%!"
           (X86_gas.string_of_constant cst) (string_of_data_size data_size); *)
...
(* Relocations: we should compute all non-local relocations completely at the
   end. We should keep the last string/bytes couple to avoid duplication. All
   external labels should be absolute (ConstLabelAbs), while internal labels
   should be replaced by a relative computation. The goal is to make all
   computations either absolute, or relative to the current offset. *)
