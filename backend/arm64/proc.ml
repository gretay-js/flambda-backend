...
let types_are_compatible left right =
  ...
  | (Int | Val | Addr), (Int | Val | Addr) | Float, Float -> true
...
let hard_vec128_reg =
  Array.map (fun r -> { r with typ = Vec128 }) hard_float_reg

let hard_float32_reg =
  Array.map (fun r -> { r with typ = Float32 }) hard_float_reg
...
let calling_conventions first_int last_int first_float last_float make_stack
  ...
  for i = 0 to Array.length arg - 1 do
    ...
    | Vec128 -> loc.(i) <- loc_vec128 last_float make_stack float ofs
    | Vec256 | Vec512 -> Misc.fatal_error "arm64: got 256/512 bit vector"
    | Float32 -> loc.(i) <- loc_float32 last_float make_stack float ofs
...
let loc_arguments arg =
  calling_conventions 0 last_int_register 100 115 outgoing
    (-size_domainstate_args) arg

let loc_parameters arg =
  let loc, _ =
    calling_conventions 0 last_int_register 100 115 incoming
      (-size_domainstate_args) arg
  in
  loc
...
let external_calling_conventions first_int last_int first_float last_float
  ...
  List.iteri
    (fun i ty_arg ->
      ...
      | XInt | XInt64 -> loc.(i) <- [| loc_int last_int make_stack int ofs |]
      | XInt32 | XInt16 | XInt8 ->
        loc.(i) <- [| loc_int32 last_int make_stack int ofs |]
      | XFloat -> loc.(i) <- [| loc_float last_float make_stack float ofs |]
      | XVec128 -> loc.(i) <- [| loc_vec128 last_float make_stack float ofs |]
      | XVec256 | XVec512 ->
        Misc.fatal_error "XVec256 and XVec512 not supported on ARM64"
      | XFloat32 -> loc.(i) <- [| loc_float32 last_float make_stack float ofs |])
    ty_args;
  loc, Misc.align !ofs 16, Cmm.Align_16
(* keep stack 16-aligned *)
...
let loc_external_results res =
  let loc, _ = calling_conventions 0 1 100 101 not_supported 0 res in
  loc
...
let destroyed_at_c_noalloc_call =
  ...
  let vec128_regs_destroyed_at_c_noalloc_call =
    (* Registers v8-v15 must be preserved by a callee across subroutine calls;
       ...
       of each value stored in v8-v15 need to be preserved [8]; it is the
...
let destroy_neon_reg n =
  [| phys_reg Float (100 + n);
     phys_reg Float32 (100 + n);
...
let destroyed_at_basic (basic : Cfg_intf.S.basic) =
  ...
  | Op
      (Static_cast
        (Int_of_float _ | Float_of_int _ | Float_of_float32 | Float32_of_float))
    ->
    [||]
  | Op (Static_cast (V128_of_scalar _ | Scalar_of_v128 _)) | Op (Intop Ipopcnt)
    ->
    if !Arch.feat_cssc then [||] else destroy_neon_reg7
  | Op
      (Intop
        ( Iadd | Isub | Imul | Idiv | Imod | Iand | Ior | Ixor | Ilsl | Ilsr
        | Iasr | Imulh _ | Iclz _ | Ictz _ | Icomp _ ))
  | Op (Int128op (Iadd128 | Isub128 | Imul64 _))
  | Op
      ( Specific _ | Move | Spill | Reload | Floatop _ | Csel _ | Const_int _
      | Const_float32 _ | Const_float _ | Const_symbol _ | Const_vec128 _
      | Stackoffset _ | Intop_imm _ | Intop_atomic _ | Name_for_debugger _
      | Probe_is_enabled _ | Opaque | Pause | Begin_region | End_region
      | Dls_get | Tls_get )
...
let destroyed_at_terminator (terminator : Cfg_intf.S.terminator) =
  ...
  | Prim { op = Probe _; _ } ->
    [||]
  | Call_no_return
      { func_symbol = _; alloc; ty_res = _; ty_args = _; stack_ofs; _ }
  | Prim
      { op =
          External
            { func_symbol = _; alloc; ty_res = _; ty_args = _; stack_ofs; _ };
...
(* CR-soon xclerc for xclerc: consider having more destruction points. We
   ...
   we could also return `true` when `destroyed_at_terminator` returns
   `destroyed_at_c_call` for instance. *)
...
let is_destruction_point ~(more_destruction_points : bool)
  ...
  | Prim { op = Probe _; _ } ->
    false
  | Call_no_return
      { func_symbol = _; alloc; ty_res = _; ty_args = _; stack_ofs = _; _ }
  | Prim
      { op =
          External
            { func_symbol = _;
...
let assemble_file infile outfile =
  ...
  Ccomp.command
    ...
    ^ dwarf_flag ^ " -o " ^ Filename.quote outfile ^ " " ^ Filename.quote infile
    )
...
let operation_supported : Cmm.operation -> bool = function
  ...
  | Cmulf Float32
  | Cdivf Float32
  | Cpackf32 | Cclz _ | Cctz _ | Cbswap _ | Capply _ | Cextcall _ | Cload _
  | Calloc _ | Cstore _ | Caddi | Csubi | Cmuli | Cmulhi _ | Cdivi | Cmodi
  ...
  | Cmulf Float64
  | Cdivf Float64
  | Ccmpf _ | Ccsel _ | Craise _ | Cprobe _ | Cprobe_is_enabled _ | Copaque
  | Cpause | Cbeginregion | Cendregion | Ctuple_field _ | Cdls_get | Ctls_get
  | Cpoll
  | Creinterpret_cast
