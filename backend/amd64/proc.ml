...
let types_are_compatible (left : Reg.t) (right : Reg.t) =
  ...
  | Vec256, Vec256 -> true
  | Vec512, Vec512 -> true
  | (Int | Val | Addr | Float | Float32 | Vec128 | Vec256 | Vec512 | Valx2), _
    ->
    false
...
let hard_int_reg =
  ...
  for i = 0 to 12 do
    v.(i) <- Reg.create_at_location Int (Reg i)
  done;
  v
...
let hard_float_reg =
  ...
  for i = 0 to 15 do
    v.(i) <- Reg.create_at_location Float (Reg (100 + i))
  done;
  v
...
let add_hard_vec256_regs list ~f =
  if Arch.Extension.enabled_vec256 () then f hard_vec256_reg :: list else list

let add_hard_vec512_regs list ~f =
  if Arch.Extension.enabled_vec512 () then f hard_vec512_reg :: list else list
...
let calling_conventions ~first_int ~last_int ~step_int ~first_float ~last_float
  ...
  (* A negative offset indicates a domainstate slot, which will generate
     unaligned moves. *)
  let align ofs size =
    if ofs <= -size then ofs else if ofs <= 0 then 0 else Misc.align ofs size
  in
...
let loc_arguments arg =
  let loc, ofs, _align =
    calling_conventions ~first_int:0 ~last_int:9 ~step_int:1 ~first_float:100
      ~last_float:109 ~make_stack:outgoing ~first_stack:(-size_domainstate_args)
      arg
  in
  loc, ofs
...
let loc_parameters arg =
  let loc, _ofs, _align =
    calling_conventions ~first_int:0 ~last_int:9 ~step_int:1 ~first_float:100
      ~last_float:109 ~make_stack:incoming ~first_stack:(-size_domainstate_args)
      arg
  in
  loc
...
let loc_external_results res =
  let loc, _ofs, _align =
    (* `~last_int:4 ~step_int:4` below is to get rdx as the second int register
       (See https://refspecs.linuxbase.org/elf/x86_64-abi-0.99.pdf, pages 21 and
       22) *)
    calling_conventions ~first_int:0 ~last_int:4 ~step_int:4 ~first_float:100
      ~last_float:101 ~make_stack:not_supported ~first_stack:0 res
  in
  loc

let unix_loc_external_arguments arg =
  calling_conventions ~first_int:2 ~last_int:7 ~step_int:1 ~first_float:100
    ~last_float:107 ~make_stack:outgoing ~first_stack:0 arg
...
let win64_loc_external_arguments arg =
  let loc = Array.make (Array.length arg) Reg.dummy in
  let reg = ref 0 and ofs = ref (if Config.runtime5 then 0 else 32) in
  for i = 0 to Array.length arg - 1 do
    match (arg.(i) : machtype_component) with
    | (Val | Int | Addr) as ty ->
      if !reg < 4
    ...
    | Vec128 | Vec256 | Vec512 ->
      (* CR mslater: (SIMD) win64 calling convention requires pass by
         reference *)
      Misc.fatal_error "SIMD external arguments are not supported on Win64"
    | Valx2 -> Misc.fatal_error "Unexpected machtype_component Valx2"
  done;
  loc, Misc.align !ofs 16, Align_16
...
let int_regs_destroyed_at_c_call_win64 =
  if Config.runtime5
  then [| 0; 1; 4; 5; 6; 7; 10; 11; 12 |]
  else [| 0; 4; 5; 6; 7; 10; 11 |]
...
let destroyed_at_c_call =
  (* C calling conventions preserve rbx, but it is clobbered by the code
     sequence used for C calls in emit.ml, so it is marked as destroyed. *)
...
let destroyed_at_large_memory_op =
  ...
  then
    (* We need a scratch register [r11] to preform the address sanitizer check,
       ...
       special calling convention via the C attribute
       [__attribute__((preserve_all))] such that all registers except for [r11]
       are callee-saved, in order to minimize the amount of spilling we need to
       do. *)
    [| rdi; r11 |]
  else [||]
...
let destroyed_at_small_memory_op =
  ...
  then
    (* Everything stated above in the comment for [destroyed_at_large_memory_op]
       ...
       in order to compute the additional [SlowPathCheck] for memory accesses
       that are smaller than one word. *)
    [| rdi; r10; r11 |]
  else [||]

let destroyed_at_single_float64_store =
  if Config.with_address_sanitizer
  then Array.append destroyed_at_small_memory_op (destroy_xmm 15)
  else destroy_xmm 15
...
let destroyed_by_simd_mem_op (instr : Simd.Mem.operation) =
  match instr with Load op | Store op -> destroyed_by_simd_op op
...
let destroyed_at_basic (basic : Cfg_intf.S.basic) =
  match basic with
  | Reloadretaddr -> destroyed_at_reloadretaddr
  | Pushtrap _ -> destroyed_at_pushtrap
  ...
  | Op
      (Load
        { memory_chunk =
            ( Byte_unsigned | Byte_signed | Sixteen_unsigned | Sixteen_signed
            | Thirtytwo_unsigned | Thirtytwo_signed | Single _ );
          _
        })
  ...
  | Op
      (Store
        ( ( Word_int | Word_val | Double | Onetwentyeight_aligned
          ...
          | Twofiftysix_unaligned | Fivetwelve_aligned | Fivetwelve_unaligned ),
          _,
          _ ))
  | Op
      (Load
  ...
  | Op (Intop (Imulh _ | Icomp _) | Intop_imm (Icomp _, _)) -> [| rax |]
  | Op (Specific (Irdtsc | Irdpmc)) -> [| rax; rdx |]
  | Op Poll -> destroyed_at_alloc_or_poll
  | Op (Alloc _) -> destroyed_at_alloc_or_poll
  | Op (Specific Ipackf32) -> [||]
  | Op (Specific (Isimd op)) -> destroyed_by_simd_op op
  | Op (Specific (Isimd_mem (op, _))) -> destroyed_by_simd_mem_op op
  | Op (Specific (Illvm_intrinsic intr)) ->
    Misc.fatal_errorf
      "destroyed_at_basic: Unexpected llvm_intrinsic %s: not using LLVM backend"
      intr
  | Op
      ...
      | Intop_imm
          ( ( Iadd | Isub | Imul | Imulh _ | Iand | Ior | Ixor | Ilsl | Ilsr
      ...
      | Specific
          ( Ilea _ | Ioffset_loc _ | Ibswap _ | Isextend32 | Izextend32
...
let destroyed_at_terminator (terminator : Cfg_intf.S.terminator) =
  ...
  | Never -> assert false
  | Always _ | Parity_test _ | Truth_test _ | Float_test _ | Int_test _ | Return
...
(* CR-soon xclerc for xclerc: consider having more destruction points. We
   current return `true` when `destroyed_at_terminator` returns `all_phys_regs`
   (which means we are conservative in the sense we will spill registers that
   would spill anyway); we could also return `true` when
   `destroyed_at_terminator` returns `destroyed_at_c_call` for instance. *)
(* note: keep this function in sync with `destroyed_at_terminator` above. *)
let is_destruction_point ~(more_destruction_points : bool)
    (terminator : Cfg_intf.S.terminator) =
  match terminator with
  | Never -> assert false
  | Always _ | Parity_test _ | Truth_test _ | Float_test _ | Int_test _ | Return
  | Raise _ | Tailcall_self _ | Tailcall_func _
  ...
  | Prim
      { op = External { func_symbol = _; alloc; ty_res = _; ty_args = _; _ };
        _
      } ->
    if more_destruction_points then true else if alloc then true else false
  | Call { op = Indirect | Direct _; _ } -> true
...
(* CR mshinwell: standardise everywhere on e.g. [contains_calls] instead of
   [fun_contains_calls] *)
...
let assemble_file infile outfile = X86_proc.assemble_file infile outfile

(* Precolored_regs is not always the same as [all_phys_regs], as some physical
   registers may not be allocatable (e.g. rbp when frame pointers are
   enabled). *)
let precolored_regs () =
...
let operation_supported = function
  ...
  | Cprefetch _ | Catomic _ | Capply _ | Cextcall _ | Cload _ | Calloc _
  | Cstore _ | Caddi | Csubi | Cmuli | Cmulhi _ | Cdivi | Cmodi | Caddi128
  | Csubi128 | Cmuli64 _ | Cand | Cor | Cxor | Clsl | Clsr | Casr | Ccsel _
  | Cbswap _ | Cclz _ | Cctz _ | Ccmpi _ | Caddv | Cadda | Cnegf _ | Cabsf _
  | Caddf _ | Csubf _ | Cmulf _ | Cdivf _ | Cpackf32 | Ccmpf _ | Craise _
  | Cprobe _ | Cprobe_is_enabled _ | Copaque | Cbeginregion | Cendregion
  | Ctuple_field _ | Cdls_get | Ctls_get | Cpoll | Cpause
  | Creinterpret_cast
      ( Int_of_value | Value_of_int | Int64_of_float | Float_of_int64
...
let trap_size_in_bytes () = if !Clflags.llvm_backend then 32 else 16
