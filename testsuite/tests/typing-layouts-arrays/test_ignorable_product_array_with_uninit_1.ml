...
(* If copying this test for a new product shape, you should only have to
   ...
   test directory. *)
type boxed_t = float * int32 * int64
type unboxed_t = #(float# * int32# * int64#)

let elem : boxed_t elem = Tup3 (float_elem, int32_elem, int64_elem)
let words_wide : int = 3
let zero () : unboxed_t = #(#0., #0l, #0L)

let to_boxed #(a, b, c) = (Float_u.to_float a, Int32_u.to_int32 b, Int64_u.to_int64 c)
let of_boxed (a, b, c) = #(Float_u.of_float a, Int32_u.of_int32 b, Int64_u.of_int64 c)
