...
let rule_of_task = function
  ...
  | Ocaml_llvm { filename; stop_after_llvmize } ->
    let common_flags =
      if stop_after_llvmize then "${stop_after_llvm_flags}" else "${common_flags}"
    in
    Format.sprintf "(run ${ocamlopt} %s.ml -c ${llvm_flags} %s)" filename common_flags
...
let dependency_of_task = function
  | Ocaml_llvm { filename; _ } | Ocaml_default filename -> Some (filename ^ ".ml")
...
|}
  ...
  let exe_rule ~deps ~output =
    asprintf "(run ${ocamlopt} %a -opaque -o %s.exe)" (pp_strings pp_space) deps output
  ;;
...
|}
  ...
  let pp_rule_template ~run ~tasks ppf () =
    let run_args =
      Option.map
        (fun output ->
          List.filter can_run tasks |> List.filter_map target_of_task, output ^ ".output")
        run
    in
...
let print_rule ~extra_subst ~buf rule_template =
  ...
  let subst label =
    match List.find_opt (fun (label', _) -> String.equal label label') extra_subst with
    | Some (_, res) -> res
    | None ->
      (match label with
       ...
       | "llvm_flags" ->
         ...
         "-llvm-backend -llvm-path ${OXCAML_CLANG} -keep-llvmir -dno-asm-comments \
...
let print_test ~extra_subst ~run ~tasks ~buf =
  let rule_template = Format.asprintf "%a" (F.pp_rule_template ~run ~tasks) () in
...
let () =
  ...
  in
  let print_test_ir_and_run_with_dep
    ~extra_dep_suffix
    ?(extra_dep_with_llvm_backend = false)
    name
    =
    ...
    print_test
      ...
      ~tasks:
        ([ (if extra_dep_with_llvm_backend
            then Ocaml_llvm { filename = extra_dep_name; stop_after_llvmize = false }
  ...
  print_test_ir_and_run_with_dep
    ~extra_dep_suffix:"defn"
    ~extra_dep_with_llvm_backend:true
    "many_args";
  ...
  print_test_c ~c_suffix:"stub" "vec128_const";
  ()
;;
