...
# Run a single test
make test-one TEST=oxcaml/tests/backend/llvmize/test_name.ml

# Regenerate dune.inc after modifying gen/gen_dune.ml
cd oxcaml/tests/backend/llvmize
dune build gen/gen_dune.exe && ./gen/gen_dune.exe > dune.inc

# Update expected output after verifying changes
make promote-one TEST=oxcaml/tests/backend/llvmize/test_name.ml
```
...
3. Regenerate dune.inc:
   ```bash
   dune build gen/gen_dune.exe && ./gen/gen_dune.exe > dune.inc
   ```

4. Create empty output files:
   ```bash
   touch test_name.output
   touch test_name_ir.output  # If applicable
   ```

5. Run tests and promote output:
   ```bash
   make runtest-llvmize
   make promote-one TEST=oxcaml/tests/backend/llvmize/test_name.ml
   ```
